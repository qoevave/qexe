...
The QExE tool currently features five quality evaluation methods. To operate a test using the QExE tool, the `TestMethod` entry in the [configuration file](../getting-started/config-file) is a mandatory setting.

Four of these methods are traditionally used in **audio quality** evaluation settings, and is indicated using the [`Modality : A`](../getting-started/config-file) flag. The remaining method can be used for **audio, visual, or audiovisual quality** evaluation and is indicated via the [`Modality : A, V, or AV`](../getting-started/config-file) flag.
